...
export function createVfxSystem({ THREE, scene, state, performance, SAFETY_LIMITS, temp, sceneResources }) {
  ...
  function spawnBurst(position, color, count, speed, life = 0.3, scale = 1) {
    ...
    for (let i = 0; i < burstCount; i++) {
      ...
      spawnVfxParticle(position, temp.vec3B, color, life * (0.8 + Math.random() * 0.5), scale * (0.65 + Math.random() * 0.65));
    }
  }
  ...
  function clear() {
    state.entities.vfxParticles.forEach((fx) => scene.remove(fx.mesh));
