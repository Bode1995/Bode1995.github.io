...
import { loadProfile, loadSelectedCharacterId, saveProfile, saveSelectedCharacterId } from './profile.js';
...
createWorldMap({ THREE, gameplayConfig, mapRoot, addCollider });
...
function getCharacterDef(characterId = selectedCharacterId) {
  return characterModule.getCharacterDef(characterId, selectedCharacterId);
}
...
function setPlayerCharacter(characterId) {
  ...
  playerRig = characterModule.createCharacterRig(def);
...
state.runPowers = runPowers;
state.ui.pickupNotices = pickupNotices;
state.entities = { enemies, bullets, damageNumbers, vfxParticles, chainBeams, powerPickups };
...
const characterSelection = setupCharacterSelection({
  ...
  isSelectedCharacter: (characterId) => characterId === selectedCharacterId,
});
...
function animate() {
  ...
  characterModule.animateCharacterRig(playerRig, moveBlend, elapsed);
