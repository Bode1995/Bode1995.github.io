...
export const UPGRADE_DEFS = [
  { id: 'baseDamage', group: 'Pilot Upgrades', label: 'Base Damage', shortLabel: 'Damage', icon: '✦', accent: '#ff9b74', description: 'Erhöht den Schaden jeder Kugel.', baseCost: 60, costStep: 35, maxLevel: 12, format: (lvl) => `${(1 + lvl * 0.22).toFixed(2)}x`, tree: { x: 0, y: 0, links: ['attackSpeed', 'maxHealth'] } },
  { id: 'attackSpeed', group: 'Pilot Upgrades', label: 'Attack Speed', shortLabel: 'Fire Rate', icon: '⟳', accent: '#ffd36f', description: 'Senkt das Feuerintervall deines Blasters.', baseCost: 70, costStep: 40, maxLevel: 10, format: (lvl) => `${Math.max(0.07, 0.18 * Math.pow(0.94, lvl)).toFixed(2)}s`, tree: { x: 1, y: 1, links: ['movementSpeed', 'upgradeCapBoost'] } },
  { id: 'maxHealth', group: 'Pilot Upgrades', label: 'Max Health', shortLabel: 'Vitality', icon: '❤', accent: '#ff7a96', description: 'Mehr Lebenspunkte pro Run.', baseCost: 80, costStep: 45, maxLevel: 12, format: (lvl) => `${100 + lvl * 14} HP`, tree: { x: 1, y: -1, links: ['shieldCapacity'] } },
  { id: 'movementSpeed', group: 'Pilot Upgrades', label: 'Movement Speed', shortLabel: 'Mobility', icon: '➜', accent: '#66ffd8', description: 'Schnelleres Traversieren der Arena.', baseCost: 55, costStep: 30, maxLevel: 12, format: (lvl) => `${(1 + lvl * 0.05).toFixed(2)}x`, tree: { x: 2, y: 2, links: ['slowDuration'] } },
  { id: 'burnDamage', group: 'Power-up Upgrades', label: 'Burn Damage', shortLabel: 'Burn', icon: '🔥', accent: '#ff8c5d', description: 'Verstärkt Feuer-DOT.', baseCost: 65, costStep: 36, maxLevel: 10, format: (lvl) => `${(1 + lvl * 0.18).toFixed(2)}x`, tree: { x: 3, y: -2, links: ['poisonDamage'] } },
  { id: 'poisonDamage', group: 'Power-up Upgrades', label: 'Poison Damage', shortLabel: 'Toxin', icon: '☠', accent: '#86ff7a', description: 'Verstärkt Gift-DOT.', baseCost: 65, costStep: 36, maxLevel: 10, format: (lvl) => `${(1 + lvl * 0.18).toFixed(2)}x`, tree: { x: 4, y: -1, links: ['rocketRadius'] } },
  { id: 'slowDuration', group: 'Power-up Upgrades', label: 'Slow Duration', shortLabel: 'Slow', icon: '❄', accent: '#90dcff', description: 'Verlängert die Wirkzeit von Ice.', baseCost: 60, costStep: 34, maxLevel: 10, format: (lvl) => `${(1.2 + lvl * 0.16).toFixed(1)}s`, tree: { x: 4, y: 2, links: ['lightningRange'] } },
  { id: 'lightningRange', group: 'Power-up Upgrades', label: 'Lightning Range', shortLabel: 'Arc', icon: '⚡', accent: '#a6b4ff', description: 'Längere Kettenreichweite für Blitz-Treffer.', baseCost: 72, costStep: 42, maxLevel: 10, format: (lvl) => `${(6.5 + lvl * 0.45).toFixed(1)}m`, tree: { x: 5, y: 1, links: ['rocketRadius'] } },
  { id: 'rocketRadius', group: 'Power-up Upgrades', label: 'Rocket Radius', shortLabel: 'Blast', icon: '☄', accent: '#ffb85c', description: 'Größere Explosionsradien.', baseCost: 72, costStep: 42, maxLevel: 10, format: (lvl) => `${(1.8 + lvl * 0.18).toFixed(1)}m`, tree: { x: 5, y: -1, links: [] } },
  { id: 'shieldCapacity', group: 'Power-up Upgrades', label: 'Shield Capacity', shortLabel: 'Shield', icon: '🛡', accent: '#82d7ff', description: 'Shields absorbieren mehr Schaden.', baseCost: 68, costStep: 38, maxLevel: 10, format: (lvl) => `${26 + lvl * 6} HP`, tree: { x: 2, y: -2, links: ['burnDamage', 'upgradeCapBoost'] } },
  { id: 'upgradeCapBoost', group: 'Control Upgrades', label: 'Upgrade Cap', shortLabel: 'Cap+', icon: '⬡', accent: '#d7b0ff', description: 'Erhöht das Max-Level aller anderen Upgrades global um +1 pro Kauf.', baseCost: 180, costStep: 110, unlimited: true, costCurve: (lvl) => 180 + (lvl * 110) + Math.round(lvl * lvl * 28), format: (lvl) => `+${lvl}`, tree: { x: 2, y: 0, links: ['burnDamage', 'slowDuration'] } },
];
