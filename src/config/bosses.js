...
function createBossDefinition(config) {
  return {
    ...
    worldIndex: config.worldIndex || 1,
    unlockWorld: config.unlockWorld || config.worldIndex || 1,
    unlockAfterLevel: config.unlockAfterLevel || 5,
...
export const BOSS_DEFS = {
  [EARTH_BOSS_ID]: createBossDefinition({
    ...
    unlockAfterLevel: 5,
    name: 'Earth Titan',
    menuLabel: 'Erd-Kampagnenabschluss',
    menuChip: 'EARTH BOSS',
...
export function getBossesForWorld(worldIndex = 1) {
  return Object.values(BOSS_DEFS).filter((boss) => boss.worldIndex === worldIndex);
}
