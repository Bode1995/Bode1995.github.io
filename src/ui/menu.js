...
export function createMenuController({ ui, profile, state, helpers, actions }) {
  ...
  function renderHomeScreen() {
    ...
    if (mission.type === 'boss') {
      ui.selectedMissionLabel.textContent = `World ${mission.world} · ${worldDef.themeName} · ${mission.name}`;
      ui.selectedMissionStatus.textContent = `Bosskampf · ${helpers.isBossMissionUnlocked(mission.id) ? 'Unlocked' : 'Locked'} · ${worldDef.hudBadge} · Spezial: ${specialAbility.name}`;
    } else {
